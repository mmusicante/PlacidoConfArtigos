...
The selection of services is usually guided by the \textit{functional}\footnotemark \footnotetext{\hl{Functional properties of a computer system are characterized by the effects it produces when given a defined input.}} requirements of the application being developed~\cite{2,decastro1,PapazoglouH06}.
...
\hl{
To the best of our knowledge, and based on the related work analysis, there are no proposals that define a service-oriented approach for the whole development of systems, considering
non-functional requirements. Although the efforts that have been made to support the new
technological proposals for the Web such as web services, in general, these
development approaches focus their processes on traditional software
engineering, with emphasis on the functional aspects of the
application.} Our method to develop service-oriented applications aims to cover this lack of proposals dealing with NFRs and gives support to real needs, as presented in a company related to the risk assessment industry. \hl{Our approach targets to deal with characteristics of the 3
areas\footnote{(i) Non-Functional Requirements; (ii) Methodology for Service-Oriented Development;
and (iii) Model-Driven Engineering} in order to propose a method for
developing service-oriented applications considering non-functional aspects.

The motivation of our method is to target the specification of non-functional aspects in the early stages of development of service-oriented  applications. Our approach specifies \textit{policies} for modeling the non-functional requirements of a service separating the specification of non-functional requirements from the main functionalities of the application. This is interesting because once the policies for a given application have been defined they can be reused and/or specialized for another application with similar requirements.
}
...
\hl{These rules were implemented into a semi-automatic tool} (the \pisodm  plugin\footnotemark \footnotetext{\hl{At each stage of transformation it is possible to include new constraints. In the more abstract stages of transformation there is complete automation in the models (i.e., from $\pi$-UseCase to $\pi$-ServiceProcess, and from $\pi$-ServiceProcess to $\pi$-ServiceComposition). On the other hand, to the transformation to specific platforms (i.e. from $\pi$-ServiceComposition to $\pi$-PEWS), a review by the analyst is required after the generation of the specification code. Thus, the method is more appropriate for those who have prior knowledge of Information Technology.}} for Eclipse).
...
\hl{Wrapping up, the industrial use case presented here  provided a real validation context for testing the usability of our methodology. The software units acting as services providig specific functions and constraints that were combined to build the business logic, were representative of the kind of target systems of our methodology. The different types of business rules could be  modelled as first class citizens and clearly assciated to the business logic of the system. Thereby, we could show  how non-functional properties can be associated both to the components (i.e., services)  and to the global business logic of the system. The methodology allowed to derive these non-functional properties thanks to the specialization and transformation of the different meta-models. 

We learned and showed that decoupling the description (specification) of non functional properties from the functional aspects, allows developers  to focus on the logic of their application, insulating them from the nitty-gritty aspects of non-functional aspects. Furthermore, it improves modularity by allowing the same application to be run under different non-functional settings, if needed. This can be interesting if business rules change or if constraints associated to services evolve.

Finally, the system FlyingPig has been incrementaly built using classis software engineering techniques. It is difficult to compare the initial time to market versus the time to market using our methodology. The company could not estimate a realistic value of this measure. We intend to evaluate the quality of experience of programmers, when they will have to build a new module. The quality of experience will be given by measuring time to market and maintenability which are the two measures important to the company. }
