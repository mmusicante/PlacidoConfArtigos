...
\newcommand{\mm}[1]{\marginpar{\textcolor{magenta}{\textbf{Martin: }#1}}}
...
\RRH{\pisodm: Building SOC Applications in the Presence of NFRs}
...
%
\authorA{\sf Khalid Belhajjame}
\affA{Universit\'e de Paris - Dauphine -- Paris, France\\
  E-mail: kbelhajj@googlemail.com}
  
\authorB{\sf Valeria~de~Castro}
\affB{Universidad Rey Juan Carlos -- M\'{o}stoles, Spain\\
  E-mail: valeria.decastro@urjc.es}

\authorC{\sf Javier~A.~Espinosa-Oviedo}
\affC{Barcelona Supercomputing Centre (BSC), Spain and
	French-Mexican Laboratory of Informatics and Automatic Control (LAFMIA), Mexico\\
  E-mail: javiera.espinosa@gmail.com}
    
\authorD{\sf Martin~A.~Musicante}
\affD{Universidade Federal do Rio Grande do Norte -- Natal, Brazil\\
  E-mail: mam@dimap.ufrn.br}

\authorE{\sf Umberto~Souza~da~Costa}
\affE{Universidade Federal do Rio Grande do Norte -- Natal, Brazil\\
  E-mail: umberto@dimap.ufrn.br}

\authorF{\sf Pl\'acido~A.~Souza~Neto}
...
\authorG{\sf Genoveva~Vargas-Solar}
\affG{Univ. Grenoble Alpes, CNRS, Institute of Engineering Univ. Grenoble Alpes (Grenoble INP), LIG, 38000 Grenoble, France
	\\
  E-mail: genoveva.vargas@imag.fr}  

\authorH{\sf Jos\'e-Luis~Zechinelli-Martini}
\affH{Universidad de las Am\'ericas-Puebla -- Cholula, Mexico and
	French-Mexican Laboratory of Informatics and Automatic Control (LAFMIA), Mexico\\
...
Khalid Belhajjame, Valeria~de~Castro, Javier~A.~Espinosa-Oviedo, Martin~A.~Musicante, Umberto~Souza~da~Costa, Pl\'acido~A.~Souza~Neto, Genoveva~Vargas-Solar, Jos\'e-Luis~Zechinelli-Martini (xxxx) `\pisodm: Building SOC Applications in the Presence of Non-Functional Requirements', {\it International Journal of Web and Grid Services}, Vol. x, No. x, pp.xxx\textendash xxx.}
...
\noindent Javier Alfonso Espinosa-Oviedo is a computer scientist specialized in the domains of databases and distributed systems. His research focus mainly on large-scale data management and processing based on service oriented architectures. He currently works as postdoctoral researcher at the Barcelona Supercomputing Centre (BSC), in the Extreme Computing Group, and is researcher at the French-Mexican Laboratory of Informatics and Automatic Control (LAFMIA), in the Database Management Group. His research interests include: Stream Processing, Urban Computing, Digital Humanities, Data Visualization. He has participated in several national and European projects (CASES EU-FP7; S2EUNET FP7-IRSES; POLIWEB PEPS CNRS), and has been invited as visiting researcher in different institutions (POLITO, Italy; UdelaR, Uruguay; NUAA, China; UDLAP, Mexico) where he has lectured on selected topics of data management.
\vs{9}

\noindent Martin A. Musicante received his B.Sc at ESLAI, Argentina, in 1988 and his M.Sc and PhD in Computer Science at Universidade Federal de Pernambuco, Brazil, in 1990 and 1996, respectively. He is full professor at Universidade Federal do Rio Grande do Norte-UFRN (Natal, Brazil). He is part of the graduate program in Computer Science at UFRN. Martin was associate researcher at LI - Université François Rabelais Tours (2002-2011) and at LIFO - Université d'Orléans, France, since 2008. 
\vs{9}
...
A recent literature review  concludes that current MDD approaches rarely deal with NFRs~\cite{Ameller201542}\footnotemark \footnotetext{The study shows that only 48 of the 129 papers considered in their systematic mapping deal with NFR. Moreover, most of them consider just specific aspects of NFR, while only 5 papers (out of 129) propose a more general approach.}.

Our work introduces $\pi$-\textit{Service Oriented Development Method} (\pisodm) to support the specification of non-functional aspects of service-oriented applications, taking into account both functional and non-functional requirements, early in the software development process\footnotemark \footnotetext{The letter $\pi$  of the name of the method comes from \textit{Policy} which is a central notion to represent  NFR's and the way they are enforced at runtime.  $\pi$ stands of the pronunciation of the first letter of the word ``policy'' which is /p/.}
...
The applicability of our proposal was tested with an industrial case study concerning risk assessment for financial companies as implemented by the ORCA System\footnotemark \footnotetext{The ORCA System is a trademark of GCP Global (www.gcpglobal.com).}. In this work we show the application of our method \pisodm to develop a service based application called \FlyingPig that provides risk assessment as a service.
...
This model  represents a business case graphically as a set of value exchanges  ($\nabla$ $\triangle$) and value activities (rounded boxes)\footnotemark \footnotetext{According to~\cite{Gordijn02valuebased}, a value activity is defined as an activity expected to be profitable to one or more actors.} performed by business actors (squared boxes).
...
BPMN~\cite{BPMN}  is a graphical representation that establishes the business process of the application through a high-level workflow\footnotemark \footnotetext{Details on BPMN (Business Process Management Notation) can be found in http://www.bpmn.org/}.
...
The \textit{$\pi$-UseCase} meta-model extends the UML Use Case meta-model to describe NFRs with  the following  concepts:  \textsc{Business service}\footnotemark 
\mm{Small caps font does not appear as such!}\footnotetext{We use the 
{\sc small caps} font for referring to classes of a meta-model.}, {\sc End consumer}, {\sc Requirement}, {\sc Use Case}, {\sc Composite Use Case}, {\sc Non-\-Func\-tion\-al Requirement}, {\sc Non-Functional} {\sc At\-tri\-bute} and {\sc Constraint}.
...
Each {\sc Con\-straint} is represented as a stereotyped ({\sf constraint})\footnotemark \footnotetext{We use the {\sf sans serif} font for referring to classes defined using a meta-model.} use case.
...
\begin{compactitem}
\item 	We  used the Eclipse Modeling Framework (EMF)\footnotemark \footnotetext{The EMF project is a modeling framework and code generation facility for building tools and other applications based on a structured data model.}  to implement the  $\pi$-Service Composition and $\pi$-{\sc Pews}  meta-models. 
From these meta-models, we  developed plug-ins to support their graphical representation.

\item	 We used  ATL\footnotemark \footnotetext{http://eclipse.org/atl/. An ATL program is basically a set of rules that define how source model elements are matched and navigated to create and initialize the elements of the target models.}
for implementing the  mappings between models.

\item 	We  used Acceleo\footnotemark \footnotetext{http://www.acceleo.org/pages/home/en} to implement  the code generation plug-in to generate executable code.
...
To validate the applicability of our method we have developed a case study concerning risk assessment for financial companies as implemented by the ORCA System\footnotemark \footnotetext{The ORCA System is a trademark of GCP Global (www.gcpglobal.com).}.%\martin{Is this a case study or a proof of concept?}
...
Other tasks include amending a ``risky situation'' as well as producing evidence to show that a specific risk has been eliminated\footnotemark \footnotetext{Risky situations include  material facts such as not facilitating access to physically disabled people in a bank agency or having an unsecured access to the premises of the company.
