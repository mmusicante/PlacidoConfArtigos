...
%..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--..--
 ...
 %Transformations in the same level are ?horizontal transformations?, and transformations between different levels are called ?vertical transformations?. There are 3 model transformations defined in $\pi$-SOD-M: $\pi$-UseCase to $\pi$--ServiceProcess; $\pi$--ServiceProcess to $\pi$-ServiceComposition; and $\pi$-ServiceComposition to $\pi$-PEWS.
...
The PIM to PIM transformation rules between $\pi$-UseCase and $\pi$-ServiceProcess meta-models (Table \ref{fig:transformationsUseCase-ServiceProcess})  specify how to express  simple and composite use cases defining the functions provided by the developed application, as a service process describing the application logic and its associated non-functional properties.  The principle is to express a set of $\pi$-use cases (i.e., use cases with constraints) related by associations of type {\sc Extend}   in terms of  actions related by a control flow and associated contracts.
\begin{figure}
\centering{
...
Every {\sf Use Case} is transformed into an {\sf Action} defined in the $\pi$-ServiceProcess meta-model, and every {\sf Extend} association identified in a $\pi$-use case model is transformed into a  {\sf Fork node}.
If the extend association has only one {\sf use case},  the {\sf action} of the fork is an alternative flow. If the {\sf Extend} association has several source {\sf use cases},  different {\sf Actions} will mutual alternative flows of the fork node.  As shown in the $\pi$-ServiceProcess  meta-model an {\sc Activity service}  consists of a composition of one or more entities of type {\sc Action}.  {\sc Contracts}  are associated to  {\sc Actions}.
...
\end{figure}
   ...
   A {\sf Constraint} of a (composite) {\sf Use Case}  is transformed into an  {\sf Assertion}.  The set of constraints  in a $\pi$-UseCase model  transformed into assertions are grouped in a {\sf Contract} in a $\pi$-ServiceProcess model (figure  \ref{fig:transformationsUseCase-ServiceProcessRules}-a).
A constraint of a given type ({\sf Constraint type}) can have different transformations as stated in the following rules (figure   \ref{fig:transformationsUseCase-ServiceProcessRules}-b):
\begin{itemize}
\item  {\sc Business Constraint type} is transformed into any {\sf Assertion} that does not consider value attributes information like maxValue and minValue;
\item   {\sc Value Constraint type} is with the  {\sf  isExceptionalBehaviour}  set to false, is transformed into one {\sf Assertion};
\item  {\sc Value Constraint type}  with the {\sf  isExceptionalBehaviour } attribute set to true, is transformed into  {\sf Exceptional behaviour};
\end{itemize}
%Given the $\pi$-UseCase meta-model, to all {\sc Constraint} entity related with a {\sc Use Case}, there is a {\sc Contract} that compounds a set of {\sc Assertions} entity  and the {\sc Use Case} is refined in a service {\sc Action}.
 ...
 For {\sf Value Constraint Type}s
  the designer must specify value thresholds associated to variables according to the  { constraint}s  and {use case}s. By default, value constraints are transformed into pre-conditions and business constraints are transformed into post-conditions. This rule may be adjusted.
...
Given the "download music" use case from the scenario example, it includes pay- ment process to buy the music. It is represented as a include in the $\pi$-UseCase model and is transformed in a sequence flow in the $\pi$-ServiceProcess model (as presented in figure \ref{fig:transformation-example-include}). 
\end{example}
...
Figure \ref{fig:ServiceProcess-ServiceComposition-Rules} presents the main transformation rules for this level. The  goal of this transformation  is to group all {\sf Contracts} and {\sf Actions} into {\sf Policies} and {\sf Service activities}, respectively.   The  criterion is that all {\sc Contracts} from the same {\sc Non-functional requirement} will be transformed into one {\sc Policy} type (e.g., safety, performance, and reliability {\sc Policy} types).
...
{\sf Actions}  are transformed into an {\sf Action} in the target model, and every {\sc Service activity} is transformed into a {\sf Service activity} in the target model. 
For the non-functional specifications, a {\sf Contract} entity with the same { non-functional requirement} in the source model ($\pi$-ServiceProcess model) is transformed into a {\sf Policy} in the target model ($\pi$-ServiceComposition model). Each {\sf Assertion} is transformed into a {\sf Rule:Condition} attribute in the target model, but if the {\sf Assertion} has a value type, the name and the attributes in the source model will be transformed into a {\sc Variable} in the target model. The other {\sf Assertion property} values remains unchanged. The {\sf Assertion: Aproperty} attribute can have different transformations, depending of the following values:
\begin{itemize}
\item {\sf Post-Conditions} are transformed into{\sf  Post} in a $\pi$-ServiceComposition model; 
\item {\sf Preconditions} are transformed into {\sf Pre} in a $\pi$-ServiceComposition model;
\item {\sf Timerestrictions} are transformed into {\sf Time} in a $\pi$-ServiceComposition model.
\end{itemize}
