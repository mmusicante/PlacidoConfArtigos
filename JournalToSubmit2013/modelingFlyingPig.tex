...
Then, ORCA suggests amendments (recommendations to change practices) and provides an evaluation report. 
...
Questionnaires are considered tasks to be performed by users. 
...
In our model, each $\pi$-use case may be associated to one or more (non-functional) constraints (depicted as colored ovals in Figure~\ref{fig:piUseCaseModel}). 
...
The restrictions associated to each $\pi$-use case are transformed into \textit{assertions} (represented by colored boxes) and may be decorated with pre- and post-conditions. 
...
%The previous model 
is now  completed with the services that provides functions of the application  \FlyingPig\.
